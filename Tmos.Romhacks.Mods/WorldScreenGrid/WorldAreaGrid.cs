...
namespace Tmos.Romhacks.Mods.Map
{
    public class WorldAreaGrid
    {
        ...
        public WorldAreaGrid(int sizeX, int sizeY)
        {
  ...
  //      {
		...
		public void UpdateScreenConnections(int x, int y)
		{
			...
			if (!cell.IsEmpty())
			{
				TmosModWorldScreen ws = cell.GetWorldScreen();
				...
				if (!cell_left.IsEmpty())
				{
					...
					cell_left.GetWorldScreen().ScreenIndexRight = wsRelativeIndex;
				}
				else
				{
				...
				if (!cell_right.IsEmpty())
				{
					...
					cell_right.GetWorldScreen().ScreenIndexLeft = wsRelativeIndex;
				}
				...
				if (!cell_up.IsEmpty())
				{
					...
					cell_up.GetWorldScreen().ScreenIndexDown = wsRelativeIndex;
				}
				else
				{
				...
				if (!cell_down.IsEmpty())
				{
					...
					cell_down.GetWorldScreen().ScreenIndexUp = wsRelativeIndex;
				}
				else
				{
			...
			else
			{
				if (!cell_left.IsEmpty())
				{
					cell_left.GetWorldScreen().ScreenIndexRight = 0xFF;
				}
				if (!cell_right.IsEmpty())
				{
					cell_right.GetWorldScreen().ScreenIndexLeft = 0xFF;
				}
				if (!cell_up.IsEmpty())
				{
					cell_up.GetWorldScreen().ScreenIndexDown = 0xFF;
				}
				if (!cell_down.IsEmpty())
				{
